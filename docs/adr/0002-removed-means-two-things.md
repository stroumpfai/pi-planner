...
`Removed` never enters a State List through import — imported Removed rows are deleted, so no surviving item carries the value and there is nothing for discovery to pick up. The only way it appears in a list is someone adding it in the States editor.
